...
# Global variables to store API key, provider, and current model
api_key = None
provider = None
current_model = None
...
async def embed_file(data: Dict[str, str]):
    ...
    try:
        ...
        logging.info(f"Data loaded for user {user_id}. Text content length: {len(text_content)}")

        text_splitter = RecursiveCharacterTextSplitter(chunk_size=500, chunk_overlap=10)
        split_data = text_splitter.split_text(text_content)
        logging.info(f"Text split for user {user_id}. Number of chunks: {len(split_data)}")
        ...
        embedding_tokens = TokenCostManager().count_string_tokens(prompt=text_content, model=embedding_model)
        ...
        # Generate questions
        query_result = await generate_document_queries(text_content)
